namespace OpenTkInputTest
{
    ...
    {
        private IReadOnlyList<JoystickState> m_windowJoystickStates;

        private readonly JoystickStateStruct[][] m_joystickStates;
        private JoystickStateStruct[] m_initialJoystickStates;
        ...
        /// </summary>
        public JoystickStateStruct[] CurrentStates => m_joystickStates[m_statePointer];

        public JoystickAdapter(IReadOnlyList<JoystickState> joystickInputs, float axisDeadzone)
        {
            m_windowJoystickStates = joystickInputs;

            m_joystickStates = new JoystickStateStruct[2][];
            m_initialJoystickStates = Array.Empty<JoystickStateStruct>();
        ...
        {
            JoystickState[] activeSticks = m_windowJoystickStates
                .Where(stick => stick != null)
                .ToArray();

            m_joystickStates[0] = new JoystickStateStruct[activeSticks.Length];
            m_joystickStates[1] = new JoystickStateStruct[activeSticks.Length];
            m_initialJoystickStates = new JoystickStateStruct[activeSticks.Length];


            for (int i = 0; i < activeSticks.Length; i++)
            {
                m_joystickStates[0][i] = new JoystickStateStruct(activeSticks[i]);
                m_joystickStates[1][i] = new JoystickStateStruct(activeSticks[i]);
                m_initialJoystickStates[i] = new JoystickStateStruct(activeSticks[i]);
            }

            // Capture initial zero
            foreach (JoystickStateStruct joystick in m_initialJoystickStates)
            {
        ...
        public void SampleJoystickStates()
        {
            ...
            foreach (JoystickStateStruct joystick in m_joystickStates[m_statePointer])
            {
                joystick.Update(m_windowJoystickStates[joystick.JoystickId]);
            }
            ...
            JoystickStateStruct currentState;
            JoystickStateStruct prevState;
            JoystickStateStruct initialState;
